...
COPY rancher-remotedialer /usr/local/bin/rancher-remotedialer
# Final Stage
# Use a minimal base image like scratch, which contains nothing, for the final image.
FROM alpine:3.20

# Copy the binary from the 'builder' stage to the final image.
COPY --from=builder /usr/local/bin/rancher-remotedialer /usr/local/bin/rancher-remotedialer
...
ENTRYPOINT ["/usr/local/bin/rancher-remotedialer"]
