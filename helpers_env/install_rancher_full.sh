...
validate_rancher_channel(){
    case "$rancher_helm_channel" in
        ...
        binary)
          echo "Setting up rancher chart channel $rancher_helm_channel..."
          echo "Binary to be installed: ../$(pwd)/rancher"
          ;;
        *)
...
install_rancher(){
    ...
    if [[ "$install_method" == "helm" ]]; then
      ...
      export_kubeconfig
      COMMAND="CATTLE_DEBUG=true CATTLE_SERVER_URL=${rancher_hostname} CATTLE_AGENT_TLS_MODE=\"system-store\" \
              CATTLE_BOOTSTRAP_PASSWORD=\"admin\" CATTLE_DEV_MODE=320 $(pwd)/rancher --add-local=true --kubeconfig ${KUBECONFIG} \
              --audit-log-path /tmp/rancher-audit.log --audit-log-maxsize 100 --audit-log-maxage 10 --audit-log-maxbackup 10 \
              --audit-level 0 --http-listen-port 8080 --https-listen-port 8443 --no-cacerts --debug"
      echo "Command:   \n${COMMAND}"
