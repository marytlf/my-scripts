...
# SAML Endpoints
# -------------------------------------------------------------------
def get_rancher_csrf():
    """Proactively fetch CSRF token from Rancher."""
    url = f"https://{RANCHER_URL}/v1/management.cattle.io.settings"
    try:
        resp = requests.get(url, verify=False, timeout=5)
        token = resp.cookies.get('CSRF')
        if token:
            print(f"[CSRF] Fetched fresh token: {token}")
            return token
    except Exception as e:
        print(f"[CSRF] Proactive fetch failed: {e}")
    return ""
...
def saml_sso():
    if request.method == 'GET':
        ...
        csrf_token = (request.headers.get('X-Api-Csrf') or 
                    request.headers.get('X-Csrf-Token') or 
                    request.cookies.get('CSRF') or "")
        
        # 2. Proactively bridge if missing
        if not csrf_token:
            csrf_token = get_rancher_csrf()
...
"""
        ...
        csrf_token_header = get_rancher_csrf() 
        username = request.form.get('username', '')
        password = request.form.get('password', '')

        print(f"[DEBUG] Processing SAML Response for user: {relay_state}")
        print(f"[DEBUG] CSRF form token being sent: '{csrf_token}'")
        print(f"[DEBUG] CSRF header token being sent: '{csrf_token_header}'")
