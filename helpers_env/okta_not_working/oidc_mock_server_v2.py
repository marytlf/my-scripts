...
def fetch_rancher_csrf(): 
    """Crawler to fetch a fresh CSRF token directly from Rancher."""
    # Ensure RANCHER_URL is just the hostname from your config
    target_url = f"https://{RANCHER_URL}/v1/management.cattle.io.settings"
    try:
        # Use a session to keep cookies organized
        session = requests.Session()
        
        # CRITICAL: allow_redirects=False ensures we see the first response where cookies are set
        response = session.get(target_url, verify=False, timeout=5, allow_redirects=False)
        
        # Check both the session jar and the raw response cookies
        cookies = session.cookies.get_dict()
        token = cookies.get('CSRF') or cookies.get('csrf')
        
        print(f">>> CRAWLER DEBUG: Status {response.status_code}")
        print(f">>> CRAWLER DEBUG: Found Cookies: {cookies}")

        if token:
            print(f">>> CRAWLER SUCCESS: {token}")
        return token
    except Exception as e:
...
def saml_sso():
    if request.method == 'GET':
        ...
        # 1. READ COOKIE NATURALLY
        # Behind Nginx on port 443, the browser WILL send this cookie.
        csrf_token = request.cookies.get('CSRF') or ""
        
        if not csrf_token:
            # If it's still missing, it means the user isn't logged into Rancher at all.
            print(">>> WARNING: No CSRF cookie found. Ensure you are accessing via port 443.")
...
<body style="font-family:sans-serif; background:#f0f2f5; display:flex; justify-content:center; align-items:center; height:100vh;">
  <div style="background:white; padding:2rem; border-radius:8px; box-shadow:0 2px 12px rgba(0,0,0,0.1); width:360px;">
    <h2>🔐 SAML Mock Login</h2>
...
"""
        ...
        # --- Final POST to Rancher ---
        # No more document.cookie hacks needed!
        return f"""
<!DOCTYPE html>
<html>
<body>
