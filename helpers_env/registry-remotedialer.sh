...
if [ -f "Dockerfile" ]; then
    ...
    docker build -t rancher-remotedialer:latest .
else
...
docker tag rancher-remotedialer:latest localhost:5000/rancher-remotedialer:latest
docker push localhost:5000/rancher-remotedialer:latest

# Check for the k3s binary
if [ -f "/usr/local/bin/k3s" ]; then
    echo "k3s cluster detected. Pulling the image..."
    sudo /usr/local/bin/k3s crictl pull localhost:5000/rancher-remotedialer:latest
